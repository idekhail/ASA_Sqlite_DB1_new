...
class UserOperation(context: Context) : SQLiteOpenHelper(context, "myDb", null, 1) {
    ...
    fun login(username: String, password: String): Cursor {
        ...
        val cols = arrayOf(ID_COL, USER_COL, PASS_COL)  // 1 الحقول في الجدول
        val selections = "$USER_COL=? AND $PASS_COL=?"  // 2  الشرط
        val args = arrayOf(username, password)          // 3  بيانات المستخدم
